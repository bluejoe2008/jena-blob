...
{
	protected BlobStorage _blobStorage;
	...
	public AbstractBlob(BlobStorage blobStorage) throws IOException
	{
		bindBlobStorage(blobStorage);
	}
	...
	protected void syncMark() throws IOException
	{
		setMark(read(new InputStreamConsumer<byte[]>()
		{
			public byte[] consume(InputStream is) throws IOException
			{
				byte[] bytes = new byte[BlobModelFactory.getBlobMarkChars()];
				int read = is.read(bytes);
				if (read > 0)
				{
